...
DATASET_PATH = "/home/uestcauto/cyy/boardband/dataset/broadband_dataset_260401.pkl"
try:
...
MODEL_WEIGHTS_PATH = "/home/uestcauto/cyy/boardband/saved_model/broadband_dataset_260401.pth"
try:
...
# --- 步骤 4: 准备测试数据并进行推理 ---
# 直接使用全部测试集来进行评估，不再随机打乱。
# 这样你修改 t 的值，就能精确按顺序查看到底层数组对应的特定样本。
ground_truth_np = Y_test.astype(np.float32)
test_input_np = X_test.astype(np.float32)

# 将 Numpy 数组转换为 PyTorch 张量并移动到 GPU
test_input_tensor = torch.from_numpy(test_input_np).to(device)

print(f"\nRunning inference on ALL {n_samples} samples...")
...
# --- 新增代码：自动创建保存图片的文件夹 ---
FIGURES_DIR = '/home/uestcauto/cyy/boardband/figures'
os.makedirs(FIGURES_DIR, exist_ok=True)
# --- 结束新增代码 ---
...
plt.savefig(os.path.join(FIGURES_DIR, "broadband_denoise_signal_260401_comparison.png"), transparent=False, bbox_inches="tight")
# 把这里的 plt.show() 暂时注释掉，否则代码会在这里“卡住”等待你关闭窗口，导致后面的频域图根本没时间生成！
# plt.show()
...
plt.legend(loc='best')
plt.grid(True)
plt.savefig(os.path.join(FIGURES_DIR, 'broadband_denoise_freq_260401_comparison.png'), transparent=False, bbox_inches="tight")
plt.show() # 在最后统一展示所有图片
