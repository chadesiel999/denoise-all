...
%% rename
folderPath = 'D:\paper\mismatch\底噪新版\DestData0608\DestData\ch1\';  % 替换为您的文件夹路径
...
% 检查是否有无法识别的文件
if any(isnan(fileFrequencies))
    disp('以下文件未能提取频率，请检查命名格式：');
    disp(fileNames(isnan(fileFrequencies))');
    error('存在无法识别频率的文件名。');
end
...
fileFrequencies = fileFrequencies(sortedIndices);
...
for i = 1:length(fileNames)
    ...
    % 按原始频率重命名，例如 25m_Ch1.csv -> C1_Sub1_0025MHz_am.csv
    newName = ['C1_Sub1_' sprintf('%04d', fileFrequencies(i)) 'MHz' ext];
...
folderPath = 'D:\paper\mismatch\底噪新版\wave_acq\square\';

N_keep = 16000;   % 取前16000个点
...
for i = 1:length(csvFiles)
    ...
    if length(wave) >= N_keep
        ...
        wave_out = wave;
        fprintf('[警告] %s 不足 %d 点，实际只有 %d 点\n', ...
            csvName, N_keep, length(wave));
    end

    % 输出txt文件名：同名，只改后缀
    [~, nameNoExt, ~] = fileparts(csvName);
    txtName = [nameNoExt, '.txt'];
    txtPath = fullfile(folderPath, txtName);
...
working_folder = 'D:\paper\mismatch\底噪新版\DestData0608\DestData\ch1\';
...
%% Generate Sub2 label from Sub1 txt 频偏处理版本
% 单音拟合版本：先估计真实频率，再拟合 DC + cos + sin
% 输入：C1_Sub1_XXXXMHz.txt
% 输出：C1_Sub2_XXXXMHz.txt
...
% 1. 参数设置
% =============================================================
fs_real = 80e9;      % 采样率 100 GSa/s
n = 10000;           % 使用的数据长度

input_folder  = 'D:\paper\mismatch\底噪新版\DestData0608\DestData\ch1';
...
% 2. 文件扫描
% =============================================================
file_structs = dir(fullfile(input_folder, 'C1_Sub1_*.txt'));
num_files = length(file_structs);

fprintf('找到 %d 个 Sub1 txt 文件。\n', num_files);
fprintf('开始处理...\n');
...
for i = 1:num_files
    ...
    input_name = file_structs(i).name;
    full_input_path = fullfile(input_folder, input_name);

    % 提取名义频率，例如 C1_Sub1_14025MHz.txt -> 14025
    tokens = regexp(input_name, 'C1_Sub1_(\d+)MHz\.txt', 'tokens');

    if isempty(tokens)
        fprintf('[跳过] 文件名无法解析频率: %s\n', input_name);
        failed_files{end+1} = [input_name, ' (文件名错误)']; %#ok<SAGROW>
        continue;
    end

    nominal_freq_mhz = str2double(tokens{1}{1});
    nominal_freq_hz  = nominal_freq_mhz * 1e6;

    % =========================================================
    % 读取数据
    % =========================================================
    try
        x = readmatrix(full_input_path);
        x = x(:);
    catch ME
        fprintf('[读取失败] %s: %s\n', input_name, ME.message);
        failed_files{end+1} = [input_name, ' (读取失败)']; %#ok<SAGROW>
        continue;
    end
    ...
    if isempty(x)
        fprintf('[跳过] 空文件: %s\n', input_name);
        failed_files{end+1} = [input_name, ' (空数据)']; %#ok<SAGROW>
        continue;
    end
    ...
    % 为了提高频率估计精度，做零填充 FFT
    % len = 500000, fs = 100G 时，原始分辨率 200 kHz
    % zero_pad_factor = 8 后，频率间隔约 25 kHz
    zero_pad_factor = 8;
    Nfft = 2^nextpow2(len * zero_pad_factor);

    win = hann(len);
    X = fft(x0 .* win, Nfft);

    f_axis = (0:Nfft-1)' * fs_real / Nfft;

    % 只在名义频率附近搜索，避免找错峰
    search_bw = 3e6;  % ±3 MHz，够覆盖 0.8 MHz 频偏

    idx_search = find(f_axis >= nominal_freq_hz - search_bw & ...
                      f_axis <= nominal_freq_hz + search_bw);

    if isempty(idx_search)
        fprintf('[失败] %s: 频率搜索范围为空\n', input_name);
        failed_files{end+1} = [input_name, ' (频率搜索失败)']; %#ok<SAGROW>
        continue;
    end

    [~, local_idx] = max(abs(X(idx_search)));
    peak_idx = idx_search(local_idx);

    fitted_freq_hz = f_axis(peak_idx);

    % =========================================================
    % 5. 用真实频率做单音最小二乘拟合
    % =========================================================
    omega = 2 * pi * fitted_freq_hz;
    ...
    % 6. 诊断输出
    % =========================================================
    raw_half_amp   = (max(x) - min(x)) / 2;
    ideal_half_amp = (max(ideal_signal) - min(ideal_signal)) / 2;

    fprintf('%s: nominal = %.6f MHz, fit = %.6f MHz, offset = %.3f kHz, Raw半幅 = %.6g, Ideal半幅 = %.6g\n', ...
        input_name, ...
        nominal_freq_hz / 1e6, ...
        fitted_freq_hz / 1e6, ...
        (fitted_freq_hz - nominal_freq_hz) / 1e3, ...
        raw_half_amp, ...
        ideal_half_amp);
    ...
    % 7. 保存 Sub2
    % =========================================================
    output_name = sprintf('C1_Sub2_%04dMHz.txt', nominal_freq_mhz);
    full_output_path = fullfile(output_folder, output_name);

    fid = fopen(full_output_path, 'w');
    if fid == -1
        fprintf('[写入失败] %s\n', output_name);
        failed_files{end+1} = [output_name, ' (写入失败)']; %#ok<SAGROW>
        continue;
    end
...
%% Generate Sub2 square label from Sub1 square txt
% 输入：C1_Sub1_XXXXMHz_square.txt
% 输出：C1_Sub2_XXXXMHz_square.txt
% 方法：先估计真实基波频率，再估计相位，最后生成理想方波标签
...
% 1. 参数设置
% =============================================================
fs_real = 100e9;      % 采样率 100 GSa/s
n = 16000;            % square 目前只取前 16000 点
...
[b_filt, a_filt] = butter(6, Wn, 'low');   % 6阶巴特沃斯低通

input_folder  = 'D:\paper\mismatch\底噪新版\wave_acq\square\';
output_folder = input_folder;
...
% 2. 文件扫描
% =============================================================
file_structs = dir(fullfile(input_folder, 'C1_Sub1_*MHz_square.txt'));
num_files = length(file_structs);

fprintf('找到 %d 个 square Sub1 txt 文件。\n', num_files);
fprintf('开始处理...\n');
...
for i = 1:num_files
    ...
    % 例如 C1_Sub1_0040MHz_square.txt -> 0040
    tokens = regexp(input_name, 'C1_Sub1_(\d+)MHz_square\.txt', 'tokens');
    ...
    % square 只有 16000 点，频率分辨率较粗，所以零填充多一点
    zero_pad_factor = 64;
    Nfft = 2^nextpow2(len * zero_pad_factor);
    ...
    % 在名义频率附近搜索基波
    % 低频时可以适当放宽，高频时 ±5MHz 足够
    search_bw = max(5e6, 0.02 * nominal_freq_hz);
    ...
    % 基波载波，用来确定方波翻转相位
    carrier = coeffs(2) * cos(omega * t) + coeffs(3) * sin(omega * t);

    % =========================================================
    % 7. 估计方波高低电平
    % =========================================================
    % 不直接用 max/min，避免噪声尖峰影响
    high_level = prctile(x, 95);
    low_level  = prctile(x, 5);

    dc_level = (high_level + low_level) / 2;
    amp_level = (high_level - low_level) / 2;

    % 生成理想 50% 占空比方波
    square_state = sign(carrier);
    square_state(square_state == 0) = 1;

    ideal_square = dc_level + amp_level * square_state;

    try
    if length(ideal_square) > 30
        ideal_square = filtfilt(b_filt, a_filt, ideal_square);
    end
    catch ME
        fprintf('[滤波警告] %s: %s，保留未滤波方波标签\n', input_name, ME.message);
    end
    ...
    % =========================================================
    raw_half_amp   = (max(x) - min(x)) / 2;
    label_half_amp = (max(ideal_square) - min(ideal_square)) / 2;

    fprintf('%s: nominal = %.6f MHz, fit = %.6f MHz, offset = %.3f kHz, Raw半幅 = %.6g, Label半幅 = %.6g\n', ...
        input_name, ...
        nominal_freq_hz / 1e6, ...
        fitted_freq_hz / 1e6, ...
        (fitted_freq_hz - nominal_freq_hz) / 1e3, ...
    ...
    % 9. 保存 Sub2
    % =========================================================
    output_name = sprintf('C1_Sub2_%04dMHz_square.txt', nominal_freq_mhz);
    full_output_path = fullfile(output_folder, output_name);
