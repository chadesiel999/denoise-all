...
{
    internal class UPO7000L : IPlatform
    {
        ...
        //private readonly String HZBANDWIDTH_LV1_NAME = "FULL";
        ...
        private IReadOnlyList<(Int32 Index, String Name)> _HZBandWidthNames = new List<(Int32 Index, String Name)>();
        private IReadOnlyList<(Int32 Index, String Name)> _LZ1GBandWidthNames = new List<(Int32 Index, String Name)>();
        private IReadOnlyList<(Int32 Index, String Name)> _LZ2GBandWidthNames = new List<(Int32 Index, String Name)>();
        ...
        private AnaChnlTimebaseIndex _MinIndex4To1 = AnaChnlTimebaseIndex.Lv100p;
        ...
        public IReadOnlyList<(Int32 Index, String Name)> GetBandWidthNames(Boolean is2G, AnaChnlCoupling anaChnlCoupling, Boolean active, Boolean isForcedRead)
        {
            ...
            {
                return is2G ? _LZ2GBandWidthNames : _LZ1GBandWidthNames;
            }
            else
            {
        ...
        public (Int32 MaxUv, Int32 MinUv) GetBiasRange(AnaChnlCoupling coupling, AnaChnlScaleIndex scale)
        {
            if (coupling == AnaChnlCoupling.DC50)
            {
                ...
                {
                    EventBroker.Instance.GetEvent<LogEventArgs>().Publish(this, new LogEventArgs("UPO7000L DC50 Only Support <= 1V!", LogLevel.Info));
                    return (0, 0);
                }
            }
            else
            {
                ...
                {
                    EventBroker.Instance.GetEvent<LogEventArgs>().Publish(this, new LogEventArgs("UPO7000L 1M Only Support <= 10V!", LogLevel.Info));
                    return (0, 0);
                }
            }
        }
        ...
        public Int32 GetDDrMaxFrameCount(Int64 storageWaveDotsCnt, AdcInterleaveMode adcInterleaveMode)
        {
            ...
            triggerAddrStart = data != null && data is Int32 ? (Int32)data : 0x1FC00_000;
        ...
        public IEnumerable<AnaChnlCoupling> GetSupportedCouplings(String serailNumber, AnaChnlStorageMode storageMode)
        {
            ...
            {
                return storageMode == AnaChnlStorageMode.Fast
                    ? Enum.GetValues<AnaChnlCoupling>().Where(o => o != AnaChnlCoupling.Gnd)
                    : Enum.GetValues<AnaChnlCoupling>();
            }
            else
            {
                return storageMode == AnaChnlStorageMode.Fast
                    ? Enum.GetValues<AnaChnlCoupling>().Skip(2).Where(o => o != AnaChnlCoupling.Gnd)
                    : Enum.GetValues<AnaChnlCoupling>().Skip(2);
            }
        }
        ...
        public String GetProductModel()
        {
            ...
            return String.IsNullOrWhiteSpace(serialnumber) || serialnumber.Length < 5 || serialnumber.Substring(1, 4) == GetSNPrefix()
                ? "UPO7204L"
                : "UPO7104L";
        }
        ...
        private static extern IntPtr Crypt_7000L(String key, String optionname);

        public String? GetOptionActiveCode(String sn, String name)
        {
            return Marshal.PtrToStringAnsi(Crypt_7000L(sn, name));
        }
        ...
        public Boolean IncludeDigitalChnl => false;

        public Boolean EnableGetOrSetScreenBrightness => false;

        public Int32 JitterMaxDataLength => 1_000_000;
        ...
        {
            var ffts = DsoPrsnt.DefaultDsoPrsnt.TryGetRange(icp => icp.Active && icp.Id.IsBaseMath()).Where(ch => ((MathPrsnt)ch).Args.Type == MathType.FFT).ToList();
            ...
            if (ffts.Count() > 0)
            {
                ...
                {
                    if (StrongTip.Default.Show(MsgTipId.Asking, MsgTipId.JitterIsNotSupportedInFFT, MessageType.Asking))
                    {
                        ffts.ForEach(ch =>
                        {
                            ch.Active = false;
        ...
        public IEnumerable<ChannelId> GetTriggerSource() => ChannelIdExt.GetTriggerSourcesWithoutDigital();
