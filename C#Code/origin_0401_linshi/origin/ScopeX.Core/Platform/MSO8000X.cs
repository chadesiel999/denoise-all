...
{
    internal class MSO8000X : IPlatform
    {
        ...
        private const String NAME_2GHZ = "2GHz";
        private const String NAME_1GHZ = "1GHz";
        ...
        private List<(Int32 Index, String Name)> _LZ8GBandWidthNames_20GMode = new List<(Int32 Index, String Name)>()
        {
            (0,NAME_8GHZ),
            (1,NAME_4GHZ),
            (2,NAME_2GHZ),
        ...
        private List<(Int32 Index, String Name)> _LZ8GBandWidthNames_10GMode = new List<(Int32 Index, String Name)>()
        {
            (0,NAME_4GHZ),
            (1,NAME_2GHZ),
            (2,NAME_1GHZ),
        ...
        private List<(Int32 Index, String Name)> _LZ8GBandWidthNames_3U = new List<(Int32 Index, String Name)>()
        {
            (0,NAME_8GHZ),
            (1,NAME_4GHZ),
            (2,NAME_500MHZ),
        ...
        private List<(Int32 Index, String Name)> _LZ8GBandWidthNames_3U_low_scale = new List<(Int32 Index, String Name)>()
        {
            (0,NAME_NULL),
            (1,NAME_NULL),
            (2,NAME_500MHZ),
        ...
        public IReadOnlyList<(Int32 Index, String Name)> GetBandWidthNames(Boolean is2G, AnaChnlCoupling anaChnlCoupling, Boolean active, Boolean isForcedRead)
        {
            if (is2G)
            {
                return _LZ8GBandWidthNames_3U_low_scale.Where(x => !String.IsNullOrEmpty(x.Name)).ToList().AsReadOnly();
            }
            return _LZ8GBandWidthNames_3U.Where(x => !String.IsNullOrEmpty(x.Name)).ToList().AsReadOnly();
            
            //if (isForcedRead == false || active == false)
            //{
            //    return null;
            //}
            if (Constants.ANA_CHNL_TYPE == AnaChnlType.ANA_8G)
            {
                ...
                {
                    return _LZ8GBandWidthNames_10GMode.Where(x => !String.IsNullOrEmpty(x.Name)).ToList().AsReadOnly();
                }
                else
                {
                    return _LZ8GBandWidthNames_20GMode.Where(x => !String.IsNullOrEmpty(x.Name)).ToList().AsReadOnly();
                }
            }
            else
            {
        ...
        public IReadOnlyList<KeyValuePair<String, Int32>> GetAnaChnlLengthSource(List<KeyValuePair<String, Int32>> source, Double timeScale, AdcInterleaveMode adcInterleaveMode)
        {
            ...
            {
                AdcInterleaveMode.Mode2To1 => (2.5e-5, 1000),
                _ => (1e-4, 1000),
            };
        ...
        public (Int32 MaxUv, Int32 MinUv) GetBiasRange(AnaChnlCoupling coupling, AnaChnlScaleIndex scale)
        {
            ...
            {
                EventBroker.Instance.GetEvent<LogEventArgs>().Publish(this, new LogEventArgs("MSO8000HD Only Support DC1M!", LogLevel.Info));
                return (0, 0);
            }
            ...
            {
                EventBroker.Instance.GetEvent<LogEventArgs>().Publish(this, new LogEventArgs("MSO8000HD Only Support <= 1V!", LogLevel.Info));
                return (0, 0);
            }
        }
        ...
        public void LoadOriginSetting()
        {
            if (Constants.ANA_CHNL_TYPE == AnaChnlType.ANA_8G)
            {
                ...
                foreach (var id in ChannelIdExt.GetAnalogs())
                {
                    ...
                    ach.Conditioning.ScaleMinIndex = AnaChnlScaleIndex.Lv5m;
        ...
        public Boolean ShowWeakTip => false;

        public Boolean DefaultHighImpedance => Constants.ANA_CHNL_TYPE != AnaChnlType.ANA_8G;
        ...
        {
            return (Int32)(channelDelay / 1e-10);
        }
