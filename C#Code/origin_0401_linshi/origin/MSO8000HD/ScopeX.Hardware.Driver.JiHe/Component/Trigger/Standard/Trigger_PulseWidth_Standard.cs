...
namespace ScopeX.Hardware.Driver
{
    ...
    {
        internal static uint GetTriggerSource_PulseWidth()
        {
            return (uint)(uint)(Hd.UIMessage?.Trigger?.Pulse?.Source ?? ChannelId.C1);
        }
        //使用internal  是为了共用
        internal static void Config_PulseWidth()
        {
            ...
            Hd.CurrProduct!.AcqBd!.WriteToAllFpga(AcqBdReg.W.TrigCtrl_TrigTypeSelectAcq, (uint)trigType);
            HdIO.WriteReg(ProcBdReg.W.TrigCtrl_EdgeSelect_Pro, (uint)polarity);
        ...
        internal static void Config_PulseWidth_MultiQulified(ITriggerTypeOptions? triggerTypeOptions, int eventIndex)
        {
            ...
            //{
            //    Hd.CurrProduct?.AcqBd?.WriteReg(AcqBdReg.W.TrigCtrl_TrigTypeSelectAcq, AcqBdNo.B3, 1 << 3);
            //}
            //else if (pulseWitdthOption!.Source == ChannelId.C3)
            //{
            //    Hd.CurrProduct?.AcqBd?.WriteReg(AcqBdReg.W.TrigCtrl_TrigTypeSelectAcq, AcqBdNo.B5, 1 << 3);
            //}
            //else if (pulseWitdthOption!.Source == ChannelId.C4)
            //{
            //    Hd.CurrProduct?.AcqBd?.WriteReg(AcqBdReg.W.TrigCtrl_TrigTypeSelectAcq, AcqBdNo.B7, 1 << 3);
            //}
