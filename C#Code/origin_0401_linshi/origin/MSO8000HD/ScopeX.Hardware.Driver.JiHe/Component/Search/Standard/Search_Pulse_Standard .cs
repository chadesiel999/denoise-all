...
namespace ScopeX.Hardware.Driver
{
    ...
    {
        internal static void Config_Pulse(ISearchTypeOptions searchTypeOption)
        {
            var option = (HdMessage.TrigPulseOptions)searchTypeOption;

            //下发使能
            //comment for JiHe_MSO7000X HdIO.WriteReg(ProcBdReg.W.search_pc_search_pro_en, 1);
            HdIO.WriteReg(ProcBdReg.W.search_pc_search_type, 0b0000_0001_000);

            UInt64 pulseWidth = (UInt64)(option?.WidthByps ?? 4000*96);
            ...
            //选择二级脉宽触发条件
            uint polarity = (uint)(option?.Polarity ?? 0);
            uint condition = (uint)(option?.Condition ?? 0);
            uint value = (polarity << 2) + condition;
            HdIO.WriteReg(ProcBdReg.W.search_pc_search_pw_set, value);
            ...
            isPositive = option!.Polarity == PulsePolarity.Positive;
            if (isPositive)
            {
