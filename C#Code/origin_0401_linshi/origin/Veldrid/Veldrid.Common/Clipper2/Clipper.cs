...
namespace Clipper2Lib
{
    public static class Clipper
    {
        ...
        public static Paths64 Union(List<PointF> subject, FillRule fillRule)
        {
            return BooleanOp(ClipType.Union, subject, fillRule);
        }
        ...
        public static Paths64 BooleanOp(ClipType clipType, List<PointF> subject, FillRule fillRule)
        {
            var solution = new Paths64();
            var temp = new List<Int32>();
            subject.ForEach(p => { temp.Add((Int32)p.X); temp.Add((Int32)p.Y); });
            var sub = new Paths64();
            sub.Add(MakePath(temp.ToArray()));

            var c = new Clipper64();
            c.AddPaths(sub, PathType.Subject);
