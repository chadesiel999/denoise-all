...
namespace Veldrid.Common.VeldridRender.EyeRender
{
    ...
    public class EyeSpriteBatch : SpriteBatch<TextureWrapper>
    {
        private EyeInfo _EyeInfo;
        private readonly GraphicsDevice _device;
        private readonly DeviceBuffer _vertexBuffer;
        private readonly DeviceBuffer _EyeInfoBuffer;
        private readonly Sampler _sampler;
        private Pipeline _pipeline;
        private readonly ResourceLayout[] _resourceLayouts;
        private Dictionary<Texture, (DeviceBuffer buffer, ResourceSet set, ResourceSet textureSet,ResourceSet eyeInfoSet)> _buffers;

        public ResourceLayout[] ResourceLayouts => _resourceLayouts;
        /// <summary>
        /// Creates a new instance of <see cref="EyeSpriteBatch"/>.
        /// </summary>
        ...
        public EyeSpriteBatch(GraphicsDevice device,
        ...
        {
            _EyeInfo = new EyeInfo();
            _EyeInfoBuffer = device.ResourceFactory.CreateBuffer(new BufferDescription((UInt32)Unsafe.SizeOf<EyeInfo>(), BufferUsage.UniformBuffer | BufferUsage.Dynamic));
            ...
            _buffers = new Dictionary<Texture, (DeviceBuffer buffer, ResourceSet set, ResourceSet textureSet,ResourceSet eyeInfoSet)>();
        ...
        public Single MaxValue { get => _EyeInfo.MaxValue; set => _EyeInfo.MaxValue = value; }
        public Single MinValue { get => _EyeInfo.MinValue; set => _EyeInfo.MinValue = value; }
        public unsafe void DrawBatch(CommandList commandList)
        {
            var matrixSize = (int)Marshal.SizeOf<Matrix4x4>();
            commandList.UpdateBuffer(_EyeInfoBuffer, 0, _EyeInfo);
            ...
            foreach (var item in _batcher)
            {
                ...
                commandList.SetGraphicsResourceSet(2, pair.eyeInfoSet);
                commandList.Draw(4, (uint)group.Count, 0, 0);
            }
        }
        internal (DeviceBuffer buffer, ResourceSet set, ResourceSet textureSet,ResourceSet eyeInfoSet) GetBuffer(Texture t, int count)
        {
            ...
            if (!_buffers.TryGetValue(t, out var pair))
            {
                ...
                pair.eyeInfoSet = _device.ResourceFactory.CreateResourceSet(new ResourceSetDescription(_resourceLayouts[2], _EyeInfoBuffer));
                _buffers[t] = pair;
            }
            else if (size > pair.buffer.SizeInBytes)
            {
                pair.set.Dispose();
                pair.buffer.Dispose();
                pair.eyeInfoSet.Dispose();
                pair.textureSet.Dispose();
        ...
        protected override void Dispose(bool disposing)
        {
            if (disposing)
            {
                ...
                _buffers.Values.ToList().ForEach(x =>
                {
                    ...
                    x.buffer?.Dispose();
                    x.set?.Dispose();
                    x.eyeInfoSet?.Dispose();
                });
                _EyeInfoBuffer?.Dispose();
            }
        }
        ...
        private static ResourceLayout[] CreateResourceLayouts(GraphicsDevice device)
        {
            ...
            elements = new ResourceLayoutElementDescription[]
            {
                new ResourceLayoutElementDescription("EyeInfo", ResourceKind.UniformBuffer, ShaderStages.Fragment),
            };
            rld = new ResourceLayoutDescription(elements);
