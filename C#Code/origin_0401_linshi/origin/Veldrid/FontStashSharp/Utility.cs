...
namespace FontStashSharp
{
	internal static class Utility
	{
			...
			ref VertexPositionColorTexture bottomLeft, ref VertexPositionColorTexture bottomRight)
		{
			...
			topLeft.TextureCoordinate = new Vector2((float)textureRectangle.X / textureSize.Width,
													(float)textureRectangle.Y / textureSize.Height);
			topLeft.Color = color;

			topRight.Position = (baseOffset + new Vector2(size.X, 0)).TransformToVector3(ref transformation, layerDepth);
			topRight.TextureCoordinate = new Vector2((float)textureRectangle.Right / textureSize.Width,
												 (float)textureRectangle.Y / textureSize.Height);
			topRight.Color = color;

			bottomLeft.Position = (baseOffset + new Vector2(0, size.Y)).TransformToVector3(ref transformation, layerDepth);
			bottomLeft.TextureCoordinate = new Vector2((float)textureRectangle.Left / textureSize.Width,
														 (float)textureRectangle.Bottom / textureSize.Height);
			bottomLeft.Color = color;

			bottomRight.Position = (baseOffset + new Vector2(size.X, size.Y)).TransformToVector3(ref transformation, layerDepth);
			bottomRight.TextureCoordinate = new Vector2((float)textureRectangle.Right / textureSize.Width,
														(float)textureRectangle.Bottom / textureSize.Height);
