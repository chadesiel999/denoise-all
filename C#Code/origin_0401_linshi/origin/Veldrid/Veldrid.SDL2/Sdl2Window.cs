...
namespace Veldrid.Sdl2
{
    public unsafe class Sdl2Window
    {
        ...
        {
            int index = SDL_GetWindowDisplayIndex(_window);
            Rectangle rect = new Rectangle();
            SDL_GetDisplayBounds(index, &rect);
            SDL_DisplayMode displayMode = new SDL_DisplayMode();
            Sdl2Native.SDL_GetDesktopDisplayMode(index, &displayMode);
            return rect.Size;
        }
        private unsafe Size GetRendererOutputSize()
        {
            SDL_Renderer render = SDL_GetRenderer(_window);
            int w = 0, h = 0;
            SDL_GetRendererOutputSize(render, &w, &h);
            return new Size(w, h);
        }
        public Size DrawableSize => GetDrawableSize();

        public Size DisplaySize => GetRendererOutputSize();
        ...
        public event Action<MouseEvent> RightMouseDown;
        ...
        public event Action<TouchFingerEvent> FingerMotion;
        public event Action<MultiGestureEvent> MultiGesture;
        private SDL_SystemCursor cursor;

        public SDL_SystemCursor Cursor
        {
            get => cursor;
            set => SetSystemCursor(value);
        }

        private void SetSystemCursor(SDL_SystemCursor cursor)
        {
            var systemcursor = Sdl2Native.SDL_CreateSystemCursor(cursor);
        ...
        private unsafe void HandleEvent(SDL_Event* ev)
        {
            switch (ev->type)
            {
                ...
                case SDL_EventType.FingerMotion:
                    SDL_TouchFingerEvent touchFingerEvent = Unsafe.Read<SDL_TouchFingerEvent>(ev);
                    break;
                case SDL_EventType.MultiGesture:
                    SDL_MultiGestureEvent multiGestureEvent = Unsafe.Read<SDL_MultiGestureEvent>(ev);
                    break;
                case SDL_EventType.DollarGesture:
                case SDL_EventType.DollarRecord:
                    SDL_DollarGestureEvent dollarGestureEvent = Unsafe.Read<SDL_DollarGestureEvent>(ev);
                    break;
                default:
                    // Ignore
                    break;
        ...
        private void HandleFingerEvent(SDL_TouchFingerEvent touchFingerEvent)
        {
            TouchFingerEvent fingerEvent = new TouchFingerEvent(touchFingerEvent.touchId, touchFingerEvent.fingerId, touchFingerEvent.x, touchFingerEvent.y, touchFingerEvent.dx, touchFingerEvent.dy, touchFingerEvent.pressure);
            switch (touchFingerEvent.type)
            {
                ...
                case SDL_EventType.FingerUp:
                    ...
                    break;
                case SDL_EventType.FingerMotion:
                    FingerMotion?.Invoke(fingerEvent);
                    break;
            }
        }
        private void HandleGestureEvent(SDL_MultiGestureEvent multiGestureEvent)
        {
            MultiGestureEvent gestureEvent = new MultiGestureEvent(multiGestureEvent.touchId, multiGestureEvent.dTheta, multiGestureEvent.dDist, multiGestureEvent.x, multiGestureEvent.y, multiGestureEvent.numFingers);
            if (multiGestureEvent.type == SDL_EventType.MultiGesture)
            {
                MultiGesture?.Invoke(gestureEvent);
            }
        }
        ...
        private void HandleMouseButtonEvent(SDL_MouseButtonEvent mouseButtonEvent)
        {
            ...
            MouseEvent mouseEvent = new MouseEvent(button, down, mouseButtonEvent.x, mouseButtonEvent.y, mouseButtonEvent.clicks);
            _privateSnapshot.MouseEventsList.Add(mouseEvent);
            if (down)
            {
                if (button == MouseButton.Right)
                {
                    RightMouseDown?.Invoke(mouseEvent);
                }
                else if (mouseEvent.Clicks == 2)
                {
                    DoubleClick?.Invoke(mouseEvent);
                }
                else
                {
