namespace ScopeX.U2
{
    ...
    public partial class PwrModulationInfoForm : FloatForm, IEmbeddableDataView, IDataExportView
    {
        public Size LastSize { get; set; }
        private Size _IndependentSize;
        public DataTableFigure TableForm { get; set; } = null;

        public PwrModulationInfoForm(PowerAnalysisPrsnt prsnt)
        {
            this.FormClosed += PwrModulationInfoForm_FormClosed;
            InitializeComponent();
            ...
            Presenter = prsnt;
            ModulationPresenter = prsnt.ModulationPrsnt.Value;
        }

        private void PwrModulationInfoForm_FormClosed(object sender, FormClosedEventArgs e)
        {
            if (TmUpdate != null)
            {
                TmUpdate.Stop();
        ...
        private PwrModulationPrsnt ModulationPresenter { get; }
        ...
        {
            var title = ScopeX.Controls.Language.LanguageManger.Instance.GetIDMessage("TiaoZhiFenXi");
        ...
        protected override void OnLoad(EventArgs e)
        {
            ...
            InitControlLang();
            UpdateView();
            LvModulation.Columns[0].Width = 180;
            LvModulation.Columns[4].Width = 140;
        ...
        private void UpdateView()
        {
            if (!DesignMode)
            {
                ...
                void Handler()
                {
                    LvModulation.BackColor = AppStyleConfig.DefaultContextBackColor.GetBrightnessColor(-0.05);
                    LvModulation.SelectedRowColor = LvModulation.BackColor;
                    var qualities = ModulationPresenter.Modulation;
                    LvModulation.BeginUpdate();

                    Int32 row = 0;
                    foreach (PwrModulationPrsnt.ModulationItems o in qualities)
                    {
                        if (row == LvModulation.Items.Count)
                        {
                            LvModulation.Items.Add(new ListViewItem(new String[LvModulation.Columns.Count]));
                        }

                        String name = LanguageHelper.GetPowerAnalysisString($"Modulation{o.Name}");
                        if (LvModulation.Items[row].Text != name)
                        {
                            LvModulation.Items[row].Text = name;
                        }
                        if (o.Name == nameof(ModulationType.PDuty) || o.Name == nameof(ModulationType.NDuty))
                        {
                            LvModulation.Items[row].SubItems[1].Text = Double.IsNaN(o.Value.Value) ? new Quantity(o.Value, Prefix.Empty, o.Unit).ToString(6, true) : $"{o.Value:F3}%";
                            LvModulation.Items[row].SubItems[2].Text = o.Mean == null ? new Quantity(o.Mean, Prefix.Empty, o.Unit).ToString(6, true) : $"{o.Mean:F3}%";
                            LvModulation.Items[row].SubItems[3].Text = o.Max == null ? new Quantity(o.Max, Prefix.Empty, o.Unit).ToString(6, true) : $"{o.Max:F3}%";
                            LvModulation.Items[row].SubItems[4].Text = o.Min == null ? new Quantity(o.Min, Prefix.Empty, o.Unit).ToString(6, true) : $"{o.Min:F3}%";
                        }
                        else
                        {
                            LvModulation.Items[row].SubItems[1].Text = new Quantity(o.Value, Prefix.Empty, o.Unit).ToString(6, true);
                            LvModulation.Items[row].SubItems[2].Text = new Quantity(o.Mean, Prefix.Empty, o.Unit).ToString(6, true);
                            LvModulation.Items[row].SubItems[3].Text = new Quantity(o.Max, Prefix.Empty, o.Unit).ToString(6, true);
                            LvModulation.Items[row].SubItems[4].Text = new Quantity(o.Min, Prefix.Empty, o.Unit).ToString(6, true);
                        }
        ...
        {
            ScModulation.Width = tableLayoutPanel1.Width - 15;
            ScModulation.Height = tableLayoutPanel1.Height;

            LvModulation.Width = tableLayoutPanel1.Width - 15;
            LvModulation.Height = tableLayoutPanel1.Height;
        }

        public List<DataTable> GetDataTables()
        {
           return new List<DataTable>() { this.GetDataTable(LvModulation, Title) };
