...
namespace ScopeX.U2
{
    [DefaultEvent("Click")]
    [DefaultValue("Text")]
    public partial class PwrLoopAnalysisInfo : UserControl, IPwrAnalysisView, IChannelInfoStyle
    {
        #region 多语言资源Key名称
                    ...
                    case nameof(Presenter.Active):
                        {
                            if (!Presenter.Active)
                            {
                                ...
                                (Program.Oscilloscope.View as DsoForm).TryRemoveLoopAnalysisUI(Presenter.LoopAnalysisPrsnt.Value);
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            string str = ScopeX.Controls.Language.LanguageManger.Instance.GetIDMessage("HuanLuFenXi");
            TextRenderer.DrawText(e.Graphics,
                str,
                ContentFont,
            ...
            //TextRenderer.DrawText(
            //    e.Graphics,
            //    Presenter.HarmonicPrsnt.Value.HarmonicNum.ToString(),
            //    ContentFont,
            //    new Rectangle(NameWidth, y, w, h),
            //    cfc,
            //    TextFormatFlags.Right | TextFormatFlags.VerticalCenter);
        ...
        private void OnBodyClicked()
        {
            ...
            (Program.Oscilloscope.View as DsoForm).MakeOperateForm(Presenter.Id.ToString() + "(" + name + ")", PointToScreen(new Point(Width / 2, 0)), PopOrientation.Above, () =>
            {
                ...
                if (Program.Oscilloscope.PwrAnalysisDictionary.TryGetValue(Presenter.Id, out poweranalysisprsnt))
                {
                    var form = new PwrLoopAnalysisForm(poweranalysisprsnt)
                    {
