...
namespace ScopeX.U2
{
    [DefaultEvent("Click")]
    [DefaultValue("Text")]
    public partial class PwrSOAInfo : UserControl, IPwrAnalysisView, IChannelInfoStyle
    {
        #region 多语言资源Key名称
                    ...
                    case nameof(Presenter.Active):
                        {
                            if (!Presenter.Active)
                            {
                                ...
                                (Program.Oscilloscope.View as DsoForm).TryRemoveSOAUI(Presenter.SOAPrsnt.Value);
                            }
                        }
                        break;
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            string str = ScopeX.Controls.Language.LanguageManger.Instance.GetIDMessage("AnQuanGongZuoQu");
            TextRenderer.DrawText(e.Graphics,
                str,
                ContentFont,
        ...
        private void OnBodyClicked()
        {
            ...
            (Program.Oscilloscope.View as DsoForm).MakeOperateForm(Presenter.Id.ToString() + "(" +name+ ")", PointToScreen(new Point(Width / 2, 0)), PopOrientation.Above, () =>
            {
                ...
                {
                    var form = new PwrSOAForm(poweranalysisprsnt)
                    {
