...
namespace ScopeX.U2
{
    ...
    public partial class SustainTimeSettingForm : FlashBorderForm, ITriggerView, IChnlView
    {
        ...
        public SustainTimeSettingForm()
        {
            InitializeComponent();
            LangKeyInit();
            //_TrigSources = ChannelIdExt.GetTriggerSources()
            //    .Where(x => x != ChannelId.Ext && x != ChannelId.AC && x != ChannelId.AuxIn).ToArray();
            ...
            HelpClick += (_, _) =>
            {
                ...
                //    return;
                //}
                HelpProcessManager.SendCommand(HelpDocumentManager.Default.GetCommand(nameof(SustainTimeSettingForm)));
            };
        }
        ...
        public TrigSustainTimePrsnt TrigPresenter { get; set; }

        ITriggerPrsnt IView<ITriggerPrsnt>.Presenter
        {
            get => TrigPresenter;
            set => TrigPresenter = (TrigSustainTimePrsnt)value;
        }
        ...
        private void AllSettingAny()
        {
            for (Int32 i = 0; i < 4; i++)
            {
                TrigPresenter.SetPosCompCondition(_TrigSources[(Int32)CbxSelectGroup.SelectValue * 4 + i], SustainTimeLevelCondition.Any);
                ChangeTholdState(((Int32)CbxSelectGroup.SelectValue) * 4, i);
            }
            TrigPresenter.SetPosCompCondition(ChannelId.Ext, SustainTimeLevelCondition.Any);
            NebThresholdExt.Enabled = TrigPresenter.GetPosCompCondition(ChannelId.Ext) != SustainTimeLevelCondition.Any;
        ...
        private void AllSettingGreaterThan()
        {
            for (Int32 i = 0; i < 4; i++)
            {
                TrigPresenter.SetPosCompCondition(_TrigSources[((Int32)CbxSelectGroup.SelectValue) * 4 + i], SustainTimeLevelCondition.GreaterThan);
                ChangeTholdState(((Int32)CbxSelectGroup.SelectValue) * 4, i);
            }
            TrigPresenter.SetPosCompCondition(ChannelId.Ext, SustainTimeLevelCondition.GreaterThan);
            NebThresholdExt.Enabled = TrigPresenter.GetPosCompCondition(ChannelId.Ext) != SustainTimeLevelCondition.Any;
        ...
        private void AllSettingLessThan()
        {
            for (int i = 0; i < 4; i++)
            {
                TrigPresenter.SetPosCompCondition(_TrigSources[((Int32)CbxSelectGroup.SelectValue) * 4 + i], SustainTimeLevelCondition.LessThan);
                ChangeTholdState(((Int32)CbxSelectGroup.SelectValue) * 4, i);
            }
            TrigPresenter.SetPosCompCondition(ChannelId.Ext, SustainTimeLevelCondition.LessThan);
            NebThresholdExt.Enabled = TrigPresenter.GetPosCompCondition(ChannelId.Ext) != SustainTimeLevelCondition.Any;
        ...
        {
            // 处理Bug：1762
            /*var enabled = TrigPresenter.GetPosCompCondition(_TrigSources[ofs + index]) != SustainTimeLevelCondition.Any;
        ...
        private void InitSrcGroupList()
        {
            ...
            var arrs = CbxSelectGroup.Items.ToList();
            arrs.Clear();
            for (Int32 i = 0; i < _TrigSources.Length / 4; i++)
            {
                var name = _TrigSources[i * 4].ToString() + "-" + _TrigSources[i * 4 + 3].ToString();
        ...
        private void RdoConditionCh1_IndexChanged(object sender, EventArgs e)
        {
            if (!_ArgToCtrl)
            {
                TrigPresenter.SetPosCompCondition(_TrigSources[((Int32)CbxSelectGroup.SelectValue) * 4], (SustainTimeLevelCondition)RdoCondition1.ChoosedButtonIndex);
        ...
        private void RdoConditionCh2_IndexChanged(object sender, EventArgs e)
        {
            if (!_ArgToCtrl)
            {
                TrigPresenter.SetPosCompCondition(_TrigSources[((Int32)CbxSelectGroup.SelectValue) * 4 + 1], (SustainTimeLevelCondition)RdoCondition2.ChoosedButtonIndex);
        ...
        private void RdoConditionCh3_IndexChanged(object sender, EventArgs e)
        {
            if (!_ArgToCtrl)
            {
                TrigPresenter.SetPosCompCondition(_TrigSources[((Int32)CbxSelectGroup.SelectValue) * 4 + 2], (SustainTimeLevelCondition)RdoCondition3.ChoosedButtonIndex);
        ...
        private void RdoConditionCh4_IndexChanged(object sender, EventArgs e)
        {
            if (!_ArgToCtrl)
            {
                TrigPresenter.SetPosCompCondition(_TrigSources[((Int32)CbxSelectGroup.SelectValue) * 4 + 3], (SustainTimeLevelCondition)RdoCondition4.ChoosedButtonIndex);
        ...
        private void RdoConditionExt_IndexChanged(object sender, EventArgs e)
        {
            if (!_ArgToCtrl)
            {
                TrigPresenter.SetPosCompCondition(ChannelId.Ext, (SustainTimeLevelCondition)RdoConditionExt.ChoosedButtonIndex);
                NebThresholdExt.Enabled = TrigPresenter.GetPosCompCondition(ChannelId.Ext) != SustainTimeLevelCondition.Any;
            }
        }
