...
InstallTypeFull=完整安装
InstallTypeOnlyApp=只安装U2主程序
ComponentsApp=U2程序
ComponentsSettings=基础环境设置
TaskBasicEnvInstall=基础环境安装
TaskSystemConfig=系统配置
TaskInstallFont=字体安装
TaskCPPInstall=VC++环境
TaskDriversInstall=硬件驱动
TaskMsedgeInstall=Msedge浏览器
TaskFirmwareUpdate=固件升级
TaskWallpaperInstall=设置锁屏和桌面壁纸
TaskDisabledSleep=禁用睡眠模式
TaskModifyOEM=修改OEM信息
TaskAutoShowSoftKeyBoard=自动显示软键盘
TaskQuitShotdown=关机时不要弹窗阻止关机
TaskUseDarkTheme=将Windows和应用显示颜色为“深色”模式
RunDotNetInstall=正在安装.Net6，可能要花费几分钟，请稍后...
RunInstallVCPP=正在安装Visual C++ 2010...
RunInstallVCPPSP1=正在安装Visual C++ 2010 SP1...
RunInstallVCPPRedist4vs=正在安装Visual C++ Redistributable for Visual Studio 2015-2022...
RunInstallPCIEDriver=正在安装PCIE驱动...
RunInstallTMCDriver=正在安装TMC驱动...
RunInstallEdge=正在安装Edge浏览器...
CodeFirmwareUpdateUp=正在升级固件：%%s，请稍后……
CodeFirmwareUpdateSuccessRestart=固件升级完成，请对设备进行断电并重启设备! ! !
CodeOldVersionCheckMsg=新旧安装包不兼容，您已安装过旧版程序，请务必确保已备份旧版程序校准文件，并卸载旧版程序后重新运行此安装包！
CodeCanNotDeleteFile=无法删除文件: %%s
